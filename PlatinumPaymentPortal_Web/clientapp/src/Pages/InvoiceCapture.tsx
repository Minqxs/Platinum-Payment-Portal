...
function InvoiceCapturePageInner({ queryKey, paymentQueryKey }: Props) {
  ...
  const [signOff, isSignOff] = useSignOffPaymentRequest();
  ...
  const initialValues: PaymentRequisitionFormValues = {
    fullname: data?.submittedBy.fullName ?? user.fullName,
    department: data?.department ?? null,
    invoiceDate: formatDate(data?.invoiceDate) ?? "",
    paymentRequestedDate: formatDate(data?.paymentDateRequested) ?? "",
          ...
          input: {
            input: {
              ...
              paymentRequestedDate:
                new Date(values?.paymentRequestedDate ?? "").toISOString() ??
                "",
  ...
  const allowedToSignOff = hasRole("MANAGER") || hasRole("ADMIN");
            ...
            <>
              ...
              {data?.isSignedOff && (
                <Box display={"flex"} width={"100%"} justifyContent={"center"}>
                  <Typography>{`Request signed off by:${data?.manager.label}`}</Typography>
                </Box>
              )}
